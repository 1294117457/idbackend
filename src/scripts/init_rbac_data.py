...
PERMISSIONS_DATA = [
    ...
    ("user:update",           "修改用户状态",   "/api/user/admin/update-status",            "user", "用户管理", 13),
    ("user:delete",           "删除用户",       "/api/user/admin/delete",                   "user", "用户管理", 14),
    ("user:assign_role",      "分配用户角色",   "/api/user/assign-roles",                   "user", "用户管理", 15),
    ("user:read_other",       "查看他人角色",   "/api/user/roles",                          "user", "用户管理", 16),
    ...
    # ===== application 学生端 =====
    ("application:create",    "统一申请",       "/api/applications",                        "application", "加分申请", 30),
    ("application:cancel",    "取消申请",       "/api/applications/cancel",                 "application", "加分申请", 32),
    ("application:touch",     "草稿续期",       "/api/applications/touch",                  "application", "加分申请", 33),
    ("application:read",      "我的申请列表",   "/api/applications",                        "application", "加分申请", 35),
    ("application:read_detail","申请详情",       "/api/applications/detail",                 "application", "加分申请", 36),

    # ===== application 审核员端 =====
    ("application:read_pending", "待审核列表",    "/api/admin/applications/my-pending",       "application", "加分申请", 37),
    ("application:read_reviewed","我已审核",      "/api/admin/applications/my-reviewed",      "application", "加分申请", 38),
    ("application:review",    "审核申请",       "/api/applications/review",                 "application", "加分申请", 40),
    ("application:revoke",    "撤回已通过申请", "/api/admin/applications/revoke",            "application", "加分申请", 43),

    # ===== proof =====
    ("application:read_detail", "证明材料列表", "/api/proof/list",                          "application", "加分申请", 45),
    ...
    ("template:list_by_cat",  "按分类列模板",   "/api/bonus-template/by-category",          "template", "模板管理", 61),
    ("template:detail",       "模板详情",       "/api/bonus-template/detail",               "template", "模板管理", 62),
    ...
    ("template:bind_rule",    "绑定规则",       "/api/bonus-template/bind-rule",            "template", "模板管理", 66),
    ("template:unbind_rule",  "解绑规则",       "/api/bonus-template/unbind-rule",          "template", "模板管理", 67),
    ...
    ("template_category:detail",         "分类详情",  "/api/template-category/detail",          "template", "模板管理", 72),
    ("template_category:delete_preview", "删除预览",  "/api/template-category/delete-preview",  "template", "模板管理", 73),
    ("template_category:create",         "创建分类",  "/api/template-category",                 "template", "模板管理", 74),
    ("template_category:update",         "修改分类",  "/api/template-category/update",          "template", "模板管理", 75),
    ("template_category:delete",         "删除分类",  "/api/template-category/delete",          "template", "模板管理", 76),

    # ===== rule =====
    ("rule:list",             "规则列表",       "/api/rule/list",                           "rule", "规则管理", 80),
    ("rule:detail",           "规则详情",       "/api/rule/detail",                         "rule", "规则管理", 81),
    ("rule:create",           "创建规则",       "/api/rule/save",                           "rule", "规则管理", 82),
    ("rule:update",           "修改规则",       "/api/rule/update",                         "rule", "规则管理", 83),
    ("rule:delete",           "删除规则",       "/api/rule/delete",                         "rule", "规则管理", 84),

    # ===== attribute =====
    ("attribute:list",        "属性列表",       "/api/rule-attribute/list",                 "rule", "规则管理", 87),
    ("attribute:detail",      "属性详情",       "/api/rule-attribute/detail",               "rule", "规则管理", 88),
    ("attribute:create",      "创建属性",       "/api/rule-attribute",                      "rule", "规则管理", 89),
    ("attribute:update",      "修改属性",       "/api/rule-attribute/update",               "rule", "规则管理", 90),
    ("attribute:delete",      "删除属性",       "/api/rule-attribute/delete",               "rule", "规则管理", 91),

    # ===== extra_info_field =====
    ("extra_info:list",       "字段列表",       "/api/extra-info-field/list",               "extra_info", "扩展字段", 100),
    ("extra_info:active",     "已启用字段",     "/api/extra-info-field/active",             "extra_info", "扩展字段", 101),
    ("extra_info:detail",     "字段详情",       "/api/extra-info-field/detail",             "extra_info", "扩展字段", 102),
    ("extra_info:create",     "创建字段",       "/api/extra-info-field",                    "extra_info", "扩展字段", 103),
    ("extra_info:update",     "修改字段",       "/api/extra-info-field/update",             "extra_info", "扩展字段", 104),
    ("extra_info:delete",     "删除字段",       "/api/extra-info-field/delete",             "extra_info", "扩展字段", 105),
    ...
    ("file:preview",          "预览文件",       "/api/file/preview",                        "file", "文件管理", 113),
    ("file:download",         "下载文件",       "/api/file/download-url",                   "file", "文件管理", 114),
    ("file:update",           "更新文件",       "/api/file/update",                         "file", "文件管理", 115),
    ("file:delete",           "删除文件",       "/api/file/delete",                         "file", "文件管理", 116),

    # ===== rbac: role 管理 =====
    ("role:list",             "角色列表",       "/api/system/role/list",                    "rbac", "权限管理", 120),
    ("role:detail",           "角色详情",       "/api/system/role/detail",                  "rbac", "权限管理", 121),
    ("role:detail_perms",     "角色权限",       "/api/system/role/permissions",             "rbac", "权限管理", 122),
    ("role:create",           "创建角色",       "/api/system/role/create",                  "rbac", "权限管理", 123),
    ("role:update",           "更新角色",       "/api/system/role/update",                  "rbac", "权限管理", 124),
    ("role:delete",           "删除角色",       "/api/system/role/delete",                  "rbac", "权限管理", 125),
    ...
    ("permission:scan",       "接口扫描",       "/api/system/permission/interfaces",        "rbac", "权限管理", 128),
    ...
    ("permission:delete",     "删除权限",       "/api/system/permission/delete",            "rbac", "权限管理", 131),
...
ROLE_PERMISSIONS = {
    ...
    "admin": [
        ...
        "user:read_my_roles",
        # application 全部
        "application:create", "application:cancel", "application:touch",
        "application:read", "application:read_detail",
        "application:read_pending", "application:read_reviewed",
        "application:review", "application:revoke",
        # score
        ...
        "template_category:delete_preview",
        ...
        # attribute 全部
        "attribute:list", "attribute:detail",
    ...
    "reviewer": [
        ...
        # 看模板（理解规则用）
        "template:list", "template:list_by_cat", "template:detail",
        # 看分类（找模板用）
    ...
    "user": [
        ...
        "user:read_my_roles",
        # 自己申请
        "application:create", "application:cancel",
        "application:touch", "application:read",
        "application:read_detail",
        # 自己成绩
        "score:read_self", "score:read_apps", "score:recalc_self",
        # 看模板
        "template:list", "template:list_by_cat", "template:detail",

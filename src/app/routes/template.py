...
async def create_template(
    ...
    """
    template = await TemplateService.create(db, storage, rich_text_service, req)
...
async def update_template(
    ...
    template = await TemplateService.update(
        db, storage, rich_text_service, template_id, req,
    )
...
async def delete_template(
    ...
    """删除模板（级联清理 template_rule 行 + 富文本文件）"""
    await TemplateService.delete(db, storage, rich_text_service, template_id)
...
async def bind_rule(
    ...
    result = await TemplateService.bind_rule(
        db, storage, rich_text_service, template_id, req.ruleId,
    )
    return R.success_resp(
...
async def unbind_rule(
    ...
    await TemplateService.unbind_rule(
        db, storage, rich_text_service, template_id, rule_id,
    )

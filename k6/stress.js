...
export const options = {
  ...
  stages: [
    ...
    { duration: '30s',  target: 0   },   // 退火
  ],

  thresholds: {
    errors: ['rate<0.05'],     // 压力下允许少量错误
    http_req_failed: ['rate<0.05'],
    // DB pool 打满后,P95 可能升高但不应超时(30s)
...
export default function(data) {
  ...
  if (scenario < 6) {
    ...
    const listRes = http.get(
      `${api.BASE_URL}/api/admin/applications?pageNum=1&pageSize=5`,
      { headers: api.authHeaders(data.adminToken) }
    );
    if (listRes.status === 200) {
      try {
        const app = JSON.parse(listRes.body).data?.list?.[0];
        if (app?.proofs?.[0]) {
